...
plants=json.load(open('plants.json'))
...
def plants_to_dropdown(plants):
	options_html = ""

	for p in plants:
		# options_html += '<option value="{id}">{name}</option>'.format(**p)
		options_html += '<option value="{:s}">{name}</option>'.format(html.escape(json.dumps(p)), **p)
		json.dumps(p)

	return options_html

def plant_cb(plant_obj):
	print("selected plant {:s}".format(plant_obj));

	plant_obj = json.loads(plant_obj)
	print(plant_obj["name"])
...
def set_javascript_bindings(browser):
	...
	bindings.SetProperty("plant_options", plants_to_dropdown(plants));

	bindings.SetFunction("plant_cb", plant_cb)
